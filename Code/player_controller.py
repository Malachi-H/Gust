...
class PlatformerController2d(Entity):
    def __init__(self, **kwargs):
        ...
        self.walking = False
        self.velocity = (
            0  # the walk diection is stored here. -1 for left and 1 for right.
        )
        self.jump_height = 4
    ...
    def update(self):
        ...
        if (
            boxcast(
                self.position
                + Vec3(self.velocity * time.dt * self.walk_speed, self.scale_y / 2, 0),
                # self.position+Vec3(sefl,self.scale_y/2,0),
                direction=Vec3(self.velocity, 0, 0),
                distance=abs(self.scale_x / 2),
                ignore=(self,),
                traverse_target=self.traverse_target,
                thickness=(self.scale_x * 0.9, self.scale_y * 0.9),
            ).hit
            == False
        ):
        ...
        ray = boxcast(
            ...
            self.down,
            distance=max(0.15, self.air_time * self.gravity),
            ignore=(self,),
            traverse_target=self.traverse_target,
            thickness=self.scale_x * 0.9,
            # debug=True
        )
    ...
    def jump(self):
        ...
        self.jump_dust = Entity(
            model=Circle(), scale=0.5, color=color.white33, position=self.position
        )
        self.jump_dust.animate_scale(3, duration=0.3, curve=curve.linear)
        self.jump_dust.fade_out(duration=0.2)
        ...
        hit_above = boxcast(
            self.position + (0, self.scale_y / 2, 0),
            self.up,
...
if __name__ == "__main__":
    ...
    camera.add_script(
        SmoothFollow(target=player_controller, offset=[0, 1, -30], speed=4)
    )
